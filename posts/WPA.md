...
To speed up the process of capturing a WPA handshake, we can use a deauth (short for deauthentication) attack. This will trigger the access point into disconnecting multiple clients, who will _hopefully_ automatically re-connect and give us a WPA handshake to capture. This is done using the `aireplay-ng` tool.
...
_Haha, I'm kidding, it's not always that easy._
