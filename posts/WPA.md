...
> 
> WPA2-PSK "cracking" is unethical, and in many cases even illegal. Make sure you have permission to mess with networks.
...
Even though I now have the password for the network, I don't have access to the cameras yet!

A quick IP scan revealed an HP ethernet switch, which had a public web dashboard showing the IP address of the camera DVR system in the school. This DVR system interface, of course, is protected by a simple login screen. After doing some research, I found the public vulnerability [CVE-2018-9995](https://nvd.nist.gov/vuln/detail/CVE-2018-9995), which happens to affect the model of camera DVR I'm working with. 

It is a very simple vulnerability, where a `Cookie` header in a request is set to `uid=admin` to trick the DVR into thinking we're already logged in as that user.

After running this command:

    root@kubuntu:~$ curl "http://IP/device.rsp?opt=user&cmd=list" -H "Cookie: uid=admin"

We get this result:

    {"result":0,"list":[{"uid":"admin","pwd":"AdminPassword", ... }]}

*`AdminPassword` wasn't actually the password, I just substituted that where the actual password appeared.*

After using the credentials `admin` and `AdminPassword` (example) to login, I had access to the IPTV cameras throughout the entire school.
...
## References
 ...
 - [https://nvd.nist.gov/vuln/detail/CVE-2018-9995](https://nvd.nist.gov/vuln/detail/CVE-2018-9995)
 - [https://ipvm.com/forums/video-surveillance/topics/exploit-provides-plain-text-credentials-for-tbk-dvrs-and-oems](https://ipvm.com/forums/video-surveillance/topics/exploit-provides-plain-text-credentials-for-tbk-dvrs-and-oems)